"""将 Task 4 海龟策略核心思想与关键优势 Markdown 转换为格式规范的 Word 文档"""
import re
        ...
        if ch == '\\':
            ...
            if cmd == 'frac':
                ...
                if i < len(s) and s[i] == '{':
                    ...
                    out.append(f'̄{s[i+1:end]}'); i = end + 1
                elif i < len(s):
                    out.append(f'̄{s[i]}'); i += 1
            elif cmd == 'hat':
                if i < len(s) and s[i] == '{':
                    end = _find_matching_brace(s, i+1)
                    out.append(f'̂{s[i+1:end]}'); i = end + 1
            else:
                # 忽略未知命令
                pass
        elif ch == '^':
...
# ---------- 配置 ----------
MD_PATH = r'C:\Users\Administrator\Desktop\量化交易：AI大模型辅助的金融交易策略\Task 4\海龟策略核心思想与关键优势.md'
OUT_DIR = r'C:\Users\Administrator\Desktop\量化交易：AI大模型辅助的金融交易策略\Task 4'
OUT_PATH = r'C:\Users\Administrator\Desktop\量化交易：AI大模型辅助的金融交易策略\Task 4\TASK4_海龟交易法则量化策略分析.docx'
...
def add_formula_paragraph(latex):
    """居中显示数学公式，Times New Roman 字体（斜体）"""
...
while i < len(lines):
    ...
    if line.startswith('|'):
        ...
        if has_separator and len(table_lines) >= 2:
            add_table_from_markdown(table_lines)
        else:
